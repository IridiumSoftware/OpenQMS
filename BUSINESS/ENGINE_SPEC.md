...
### OQ-121 — Non-technical UX batch (compliance-architecture P3)

- **Tier:** Gap
- **Evidence type:** example-tested
- **Status:** :tested
- **Depends_on:** OQ-020, OQ-023, OQ-060, OQ-119, OQ-120
- **Claim:** Open QMS ships the P3 forward-work priority (better non-technical UX) at v0.57.0 as a single batched release. **5 new GitHub Issue Forms** under `.github/ISSUE_TEMPLATE/`: `training-completion.yml` (record training completion per ISO 13485 §6.2.2 + 21 CFR §820.25(b)) + `document-review.yml` (periodic doc review with 3-outcome flow: no-change / minor / major) + `access-review.yml` (quarterly access review per IDENTITY-MAPPING-SOP §4) + `restoration-test.yml` (annual restoration test per BACKUP-RESTORE-SOP §4 with 13-step runbook outcomes) + `regulatory-review.yml` (per-module independent regulatory review per `BUSINESS/regulatory_review_cadence.md` §2-3). **5 new role + process guides** under `docs/guide/`: `onboarding.md` (first-30-days checklist for new hires + new contractors with day-1 / day-2-7 / week-2 / week-3-4 / month-1 completion stages) + `role-quality-manager.md` (daily/weekly/monthly/quarterly/annual cadence with linked workflows) + `role-engineer.md` (commit signing + Signature-Meaning trailer guide + when-to-use-which-trailer table + per-change-type workflow + anti-pattern list) + `document-routing.md` (CODEOWNERS-based review-chain reference; traditional-eQMS-concept → GitHub-native-equivalent table; example CODEOWNERS structure for an adopter; branch-protection rules that make CODEOWNERS load-bearing; edge cases) + `capa-lifecycle.md` (7-state lifecycle state machine: OPEN → INVESTIGATION → ROOT-CAUSE → ACTION PLANNED → ACTION EXECUTED → EFFECTIVENESS CHECK → CLOSED + anti-patterns + aggregation at Management Review). Cross-linkage: every new guide references the appropriate Signature-Meaning trailer convention + OQ-XXX spec entries + relevant SOP templates from OQ-120.
- **Notes:** Shipped at v0.57.0. Closes compliance-architecture forward-work P3 (medium effort per v0.54.0 distribution). No new modules; no new registry standards. Pattern parallel to OQ-120: ships adopter-facing artifacts (issue templates + guides) without changing engine code surface. The 5 new issue templates align 1:1 with the SOPs shipped at OQ-120 (access-review.yml ↔ IDENTITY-MAPPING-SOP §4; restoration-test.yml ↔ BACKUP-RESTORE-SOP §4; regulatory-review.yml ↔ regulatory_review_cadence.md §3) — completing the lifecycle from SOP-template to operating-record-form. The 5 new guides plus the existing 12 give 17 adopter guides covering onboarding through audit. **Forward-work status after v0.57.0:** 8 priorities closed (P1 + P3 + P6 + P7 + P8 + P12 + P13 + P14); 6 priorities remain open (P2 + P15 hard; P4 + P5 + P9 + P10 + P11 medium).

### OQ-120 — Light-batch hardening release (compliance-architecture P6 + P7 + P8 + P12 + P13 + P14)

- **Tier:** Gap
- **Evidence type:** example-tested
- **Status:** :tested
- **Depends_on:** OQ-022, OQ-023, OQ-060, OQ-067, OQ-119
- **Claim:** Open QMS ships the 6 light-effort forward-work priorities identified in `docs/compliance-architecture.md` as a single hardening release at v0.55.0. **P6 (pinned + hashed deps):** `engine/uv.lock` generated by `uv lock` (10 packages, hashed); CI workflow `engine-tests.yml` migrated from `pip install -e './engine[dev]'` to `uv sync --frozen` with lockfile-presence as a hard CI gate. **P7 (signed release tags):** `docs/guide/release.md` rewritten to mandate `git tag -s` for all releases going forward; tag-verification step embedded in P8's release-artifact workflow (`git tag -v` output captured to artifact + warning emitted on failure). **P8 (CI log archival per release):** new `.github/workflows/release-artifact.yml` workflow triggered on `v*` / `release-*` tag push; captures `openqms trace --all` + `openqms coverage --all` + `openqms signatures export` + pytest log + lint output + module-count snapshot + tag-signature verification into a release-artifact ZIP attached to the GitHub Release; 90-day default retention with adopter-mirroring pattern documented. **P12 (HR-to-GitHub identity-mapping SOP):** `templates/qms-policy/IDENTITY-MAPPING-SOP-TEMPLATE.md` shipped — 9-section comprehensive SOP covering register schema (12 fields) + new-hire provisioning + quarterly access review + key rotation (planned + emergency) + offboarding + audit-log retention + linkage to OQ-020/OQ-022/OQ-023/OQ-060/OQ-119. **P13 (backup/restore SOP + restoration-test discipline):** `templates/qms-bcms/BACKUP-RESTORE-SOP-TEMPLATE.md` shipped — 8-section comprehensive SOP covering 3-2-1 backup posture (Primary + 3 Mirrors) + cron automation examples + RTO/RPO/MAO/MBCO declaration framework + 13-step annual restoration-test runbook + format-stability planning (parallel to ATMP 30-year clauses) + backup-failure escalation matrix. **P14 (regulatory-review cadence):** `BUSINESS/regulatory_review_cadence.md` shipped — 8-section process governance doc covering per-standard-kind cadence schedule (annual for active major; on-revision for revisions; quarterly for pre-effective drafts; biannual for sunsetting) + 6-criterion reviewer-qualification standard + finding-log format with 3-tier severity (minor / major / critical) + per-module attestation badge format + initial review-debt baseline (0/116 reviewed — honest baseline) + 8-module first-tier review-target list.
- **Notes:** Shipped at v0.55.0 as a single batched release per the v0.54.0 effort-distribution observation that the 6 light items could batch as a single hardening release. No new modules; no new registry standards. Lockfile under `engine/uv.lock` (10 packages); CI workflow asserts lockfile presence + uses `uv sync --frozen` for reproducible installs. Tag-verification + artifact archival under `.github/workflows/release-artifact.yml`. SOP templates land in `templates/qms-policy/` and `templates/qms-bcms/`. Process governance doc in `BUSINESS/` per audit + companion precedent. **Forward-work status after v0.55.0:** 6 priorities closed (P6 + P7 + P8 + P12 + P13 + P14); 7 priorities remain open (P2 hard / P3 + P4 + P5 + P9 + P10 + P11 medium); cross-reference `docs/compliance-architecture.md` forward-work section for updated table.
...
- **Evidence type:** example-tested
- **Status:** :tested
- **Depends_on:** OQ-022, OQ-023, OQ-060, OQ-067, OQ-015, OQ-080
- **Claim:** Open QMS ships a comprehensive trust-gate document at `docs/compliance-architecture.md` that addresses the 10 highest-frequency adopter-evaluator concerns: 21 CFR Part 11 + EU GMP Annex 11 + ISO 13485 expectations + audit trails + e-signatures + permissions + validation (engine + module) + backup/recovery + record retention + cybersecurity + system administration controls. For each topic the document maps the regulatory requirement → architecture component → spec entry → evidence file → adopter responsibility, and includes a §"How to use this document at audit" workflow + honest "What Open QMS is NOT" disclaimers reinforcing OQ-080. Closes the v0.51.0 adopter-feedback P1 (compliance architecture documentation) — the trust-gate prerequisite for serious quality-leader adoption.
- **Notes:** Shipped at v0.52.0. Per-topic structure intentionally pointer-heavy — the document is an index over existing spec entries + modules + templates + 12 docs/guide pages, not a re-claim of evidence. Includes cross-references to README + docs/modules-catalog.md + BUSINESS/ENGINE_SPEC.md + BUSINESS/artifact_registry.md + BUSINESS/dashboard.md + BUSINESS/changelog.md + BUSINESS/audit_2026-05-25_v0_50.md. Forward-work section originally acknowledged adopter-feedback priorities P2 (validation package templates) + P3 (non-technical UX) + P4 (opinionated startup-stage presets). **Expanded at v0.53.0** to incorporate independent reviewer technical assessment: added P5 (replace shell/grep doc-control with schema-validated parser + hard-fail version drift + signed audit artifact per PR), P6 (lockfile with hashed deps), P7 (signed release tags), P8 (CI log archival), P9 (template schema validation), P10 (negative-path test suite), P11 (adopter-deployment verification script), P12 (HR-to-GitHub identity-mapping SOP template), P13 (backup/restore SOP template), P14 (third-party regulatory review cadence). Also added "what reviewer flagged that is already addressed" table for transparency (5 items: README version drift fixed v0.51.0; immutable audit export shipped v0.7.0 OQ-060; module YAML schema validation shipped v0.40.0; bidirectional traceability `:verified` at OQ-001; standards-licensing posture covered at OQ-070/OQ-071).

### OQ-118 — Cross-overlay batch (4, final): automotive-supply-chain + clinical-trial-multi-region + banking-resilience + utility-cybersecurity

- **Tier:** Module
- **Evidence type:** example-tested
- **Status:** :tested
- **Depends_on:** OQ-011, OQ-040, OQ-076, OQ-088, OQ-092, OQ-094, OQ-095, OQ-098, OQ-108, OQ-109, OQ-110, OQ-111, OQ-112, OQ-113
- **Claim:** 4 final cross-overlays at v0.50.0 — completes the cross-overlay shape (introduced v0.44.0 OQ-111; extended v0.47.0 OQ-113) across the remaining major vertical intersections. **automotive-supply-chain** (automotive + automotive-asil-d + automotive-cal-4 + recall-workflow) — Development Interface Agreement (DIA) per ISO 26262-8 §5 + Cybersecurity Interface Agreement (CIA) per ISO/SAE 21434-7 + AIAG PPAP 4th-Edition 18-element + 5-level submission framework + ISO/SAE 21434 §15 supplier management + UN R155 Type-Approval + CSMS + R156 SUMS + production-suspension + NHTSA Part 573 5-working-day recall coordination + IATF 16949 §8.4 tier-N flow-down + software-supplier special considerations (OTA + AD + ADAS + RTOS). **clinical-trial-multi-region** (pharma + privacy + hipaa + digital-health-multi-region) — ICH E6(R3) GCP (Step 4 2024-01-19) + EU CTR 536/2014 + CTIS portal + 21 CFR 312 IND + §312.32 Safety Reports 15-day + 7-day fatal/life-threatening + DCT considerations (E6(R3) Annex 2 + FDA Sept 2024 final + EMA 2022) + multi-region PHI coordination (HIPAA Authorization §164.508 + GDPR Article 9 + state-law sensitive-data consent + Common Rule 45 CFR 46) + parallel IRB/IEC tracking (US single-IRB + EU CTR MS coordinated assessment + UK HRA REC + Canada REB + Swiss Cantonal Ethics) + Risk-Based Monitoring (E6(R3) Principle 4) + pharmacovigilance multi-region triage (FDA + EMA EudraVigilance + MHRA + Health Canada + PMDA + DSUR per E2F + annual IND per §312.33). **banking-resilience** (iso-37301-financial-services + dora + dora-non-ctpp + nist-csf + nist-csf-tier-3 + iso-22301) — DORA + FFIEC IT Handbook cross-regime alignment + NIST CSF 2.0 6-Function backbone (Govern + Identify + Protect + Detect + Respond + Recover) + multi-stream incident reporting (DORA 4h+72h+1mo + FRB/FDIC/OCC 36-hour CSI rule effective 2022-05-01 + NYDFS Part 500 72-hour + CIRCIA-eligible) + TLPT framework alignment (TIBER-EU + CBEST + Singapore MAS AASE + Hong Kong iCAST) + CTPP designation + Lead Overseer + OCC Third-Party Risk Bulletin 2013-29 + 2020-10 + 2023-17 + SR 11-7 + 13-19 + ISO 22301 BIA + RTO/MAO/MBCO/RPO + DORA Article 11 ICT BCP + Article 12 restoration testing. **utility-cybersecurity** (manufacturing + iso-27001 + nist-csf + nist-csf-tier-3) — NERC CIP-002 through CIP-014 BES Cyber System framework (FERC-approved under Section 215 Federal Power Act) + TSA Pipeline SD 02 series (post-Colonial Pipeline May 2021; 24-hour CISA reporting) + CISA 16 Critical Infrastructure Sectors per PPD-21 + EU NIS2 Directive 2022/2555 Essential + Important Entities (transposition 2024-10-17; Article 23 early-warning 24h + 72h + 1mo) + IEC 62443 series IACS framework + NIST SP 800-82 Rev 3 (2023) OT Security + Purdue Reference Model + CIRCIA 72h cyber + 24h ransomware reporting + Energy Subsector + ONG-C2M2 + ES-C2M2 maturity models + E-ISAC/WaterISAC/Tx-ISAC information sharing. All 4 reuse parent-overlay templates. **Composite validates** for each (automotive 5-module + pharma 5-module + banking 8-module + utility 5-module).
- **Notes:** Shipped at v0.50.0 (the **final cross-overlay batch** — completes the cross-overlay shape coverage across the major vertical intersection space prior to polishing). Registry +5 PUBLIC standards (ICH E6(R3) + EU CTR 536/2014 + NERC CIP + EU NIS2 + FFIEC IT Handbook). Cross-overlay count 8 → 12. Total module count 112 → 116. Particularly natural compositions: `automotive + automotive-asil-d + automotive-cal-4 + recall-workflow + automotive-supply-chain` (tier-1 supplier serving OEM with safety + cyber scope); `pharma + privacy + hipaa + digital-health-multi-region + clinical-trial-multi-region` (DCT sponsor running US + EU trials); `iso-37301 + iso-37301-financial-services + dora + dora-non-ctpp + nist-csf + nist-csf-tier-3 + iso-22301 + banking-resilience` (multi-jurisdiction bank with US-EU presence); `manufacturing + iso-27001 + nist-csf + nist-csf-tier-3 + utility-cybersecurity` (electric utility BES operator + EU NIS2 essential entity). Cross-overlay shape now broadly covers: medical-device intersections (combination-product + connected-medical-device + digital-health-multi-region) + pharma intersections (combination-product + cell-therapy-supply-chain + clinical-trial-multi-region) + food intersections (food-pharma-grade + food-allergen-recall) + automotive intersections (automotive-supply-chain) + financial intersections (banking-resilience) + utility intersections (utility-cybersecurity) + defense intersections (defense-aerospace-cyber) + IMS (integrated-management-system). Naturally completes the cross-overlay tour of the project's vertical + cross-cutting landscape.

### OQ-117 — `openqms jurisdictions-query` — registry standards by jurisdiction

- **Tier:** Architecture
- **Evidence type:** example-tested
- **Status:** :tested
- **Depends_on:** OQ-014
- **Claim:** Open QMS engine ships a `openqms jurisdictions-query --jurisdiction <ID> [--format json|md|text]` CLI subcommand that queries the registry for standards applicable to a given jurisdiction. Inference based on publisher → jurisdiction mapping (hardcoded `_PUBLISHER_TO_JURISDICTIONS` table in `engine/openqms/cli.py` covering the major regulatory publishers: FDA / EPA / DOT / NHTSA / FAA / DoD / HHS / OSHA / USDA-FSIS / USP / California CPPA / EU / EMA / EASA / ISO / ICAO / IATA / IMO / UNECE / OTIF / OECD / AICPA / PCI SSC / HITRUST / NIST / RTCA / SAE / AIAG / IATF / Codex / ICH / PIC-S / WHO). Forward-compatible with explicit `jurisdictions:` field on registry standards entries (none today; field is honored). Outputs JSON / Markdown / text. Supports adopter planning of per-jurisdiction module scope.
- **Notes:** Shipped at v0.49.0. Tests at `engine/tests/test_jurisdictions_query.py` covering FDA + EU MDR + unknown jurisdiction + text format. CI gate asserts FDA + EU MDR queries return >0 standards each. At v0.49.0 audit time FDA query returns 18 standards. Publisher-inference approach avoids per-standard jurisdictions field migration (130+ standards) while delivering useful adopter-planning capability.
...
- **Evidence type:** example-tested
- **Status:** :tested
- **Depends_on:** OQ-011, OQ-067
- **Claim:** Open QMS engine ships a `openqms crosswalk --module <M1> --module <M2> [...] [--format json|md] [--output FILE]` CLI subcommand that identifies clauses referencing the same standard + section across multiple modules. Output: per-(standard, section) crosswalk groups + per-module shared-vs-own-only clause counts. Useful for detecting overlap across compose-partner modules + per-jurisdiction implementations + cross-overlay parent/child relationships.
- **Notes:** Shipped at v0.49.0. Tests at `engine/tests/test_crosswalk.py` covering require-2-modules error path + summary + per-module clause-count consistency + markdown format. CI gate exercises a 3-module crosswalk (privacy + us-state-privacy + hipaa). At v0.49.0 there are 0 cross-module overlaps in the privacy-adjacent triad (each module has its own clause-IDs + section references — by design); the command is useful for spotting future overlap when it arises.
...
- **Evidence type:** example-tested
- **Status:** :tested
- **Depends_on:** OQ-011, OQ-013, OQ-067
- **Claim:** Open QMS engine ships a `openqms coverage [--module M | --all] [--format json|md] [--output FILE] [--threshold N]` CLI subcommand reporting per-module + aggregate coverage metrics. Per-module: % clauses bound to ≥1 template (clause coverage); % declared standards with ≥1 clause (standard coverage); average templates-per-clause depth; orphan clause + template lists. Aggregate: same metrics across the set + average per-module coverage. `--threshold N` exits 1 if any in-scope module's clause-coverage % < N (CI regression gate). At v0.49.0 audit time aggregate coverage = 100.0% across 112 modules / 838 clauses / 0 orphans (paired complement to the zero-orphan trace invariant).
- **Notes:** Shipped at v0.49.0. Tests at `engine/tests/test_coverage.py` covering --all summary + per-module shape + threshold-zero never-fails + markdown format. CI gate asserts aggregate clause-coverage = 100.0% on every push (regression catch). Complements OQ-067 `openqms trace --all` zero-orphan invariant from a coverage-percentage angle.

### OQ-114 — US State Privacy Umbrella cross-cutting overlay (26th cross-cutting; VCDPA-template family covering ~14 US states)

- **Tier:** Module
- **Evidence type:** example-tested
- **Status:** :tested
- **Depends_on:** OQ-011, OQ-012, OQ-014, OQ-108
- **Claim:** Open QMS ships a US State Privacy Umbrella cross-cutting overlay at `modules/us-state-privacy/module.yaml` (version 0.1.0) — 26th cross-cutting overlay; single overlay covering ~14 US state comprehensive privacy laws using the VCDPA template family + state-divergence handling. Distinct from the `privacy` cross-cutting overlay (GDPR + CCPA) because CCPA scope is California-only + the other ~14 state laws use a 2-way GDPR-like controller/processor framework distinct from CCPA's 4-way business/service-provider/contractor/third-party classification + the VCDPA-template states include unique provisions (right-to-appeal denial; UOOM mandatory in 8 states; Maryland MODPA data-minimization strict-necessity standard). 10 clauses across applicability thresholds + controller/processor framework + 6 common consumer rights with right-to-appeal + sensitive data divergence + Universal Opt-Out Mechanism (GPC honoring) per-state recognition + Data Protection Assessment (state-level DPA parallel to GDPR DPIA) + cure period + AG enforcement framework + controller obligations + **Maryland MODPA stricter posture** + **Florida FDBR narrow scope**. Covers Virginia VCDPA + Colorado CPA + Connecticut CTDPA + Utah UCPA + Texas TDPSA + Oregon OCPA + Montana MCDPA + Iowa ICDPA + Delaware DPDPA + New Hampshire NHPA + New Jersey NJDPA + Tennessee TIPA + Minnesota MCDPA + Maryland MODPA + Indiana INCDPA + Florida FDBR. Ships 2 new templates: US-STATE-PRIVACY-MATRIX (state-by-state divergence tracker with applicability thresholds + cure periods + UOOM recognition + sensitive-data scope + penalty caps + response deadlines + MODPA-specific stricter scrutiny + FDBR narrow-scope analysis) + DATA-PROTECTION-ASSESSMENT (state-level DPA — heightened-risk processing identification + safeguard assessment + risk-benefit balance + MODPA data-minimization assessment + FDBR scope check; coordinated with GDPR DPIA + CCPA Risk Assessment when those regimes also apply). Plus 2 extensions to existing privacy templates (PRIVACY-POLICY + DPA). **7-module privacy/healthcare mega composite validates**: privacy + us-state-privacy + hipaa + iso-27001 + iso-27001-privacy + hitrust-csf + hitrust-r2.
- **Notes:** Shipped at v0.48.0. Registry +1 PUBLIC standard (US State Privacy Laws). Cross-cutting overlay count: 25 → 26. Total module count 111 → 112. Particularly natural compositions: `privacy + us-state-privacy` (CCPA California + 14 other states; most common); `privacy + us-state-privacy + hipaa` (full US privacy + healthcare); `privacy + us-state-privacy + hipaa + iso-27001 + iso-27001-privacy` (ISMS + PIMS + multi-state privacy + HIPAA — strongest privacy posture for US-operating digital-health SaaS). Critical Maryland MODPA divergence (effective 2025-10-01) — data minimization "strictly necessary" standard for sensitive data + absolute prohibition on sale of sensitive data + absolute prohibition on targeted advertising to under-18 — treat MODPA as a separate compliance posture. Florida FDBR — narrowest US state law; effectively only Google + Meta + Amazon + Apple; most adopters OUT. **Forward work** per us-state-privacy README: per-state appeal procedure templates; Universal Opt-Out Signal handling SOP (GPC + IAB GPP + DAA AppChoices); Maryland MODPA data-minimization assessment template; Florida FDBR threshold determination workflow; state-AG complaint response template; Washington My Health My Data Act (MHMDA) standalone overlay.

### OQ-113 — Cross-overlay batch (5): connected-medical-device + cell-therapy-supply-chain + food-allergen-recall + defense-aerospace-cyber + digital-health-multi-region

- **Tier:** Module
- **Evidence type:** example-tested
- **Status:** :tested
- **Depends_on:** OQ-011, OQ-040, OQ-058, OQ-092, OQ-093, OQ-098, OQ-104, OQ-108, OQ-111, OQ-112
- **Claim:** 5 new cross-overlays at v0.47.0 extending the cross-overlay shape (v0.44.0 OQ-111) to additional vertical intersections. **connected-medical-device** (medical-devices + regulated-ai + privacy + iso-27001 + hipaa) — FDA §524B cyber-device cybersecurity per Sept 2023 Final Guidance + EU MDR Annex I §17.2 + MDCG 2019-16 + FDA AI/ML SaMD Predetermined Change Control Plan + postmarket cybersecurity per FDA Guidance + 21 CFR 806 corrections + NTIA-elements Software Bill of Materials (SBOM) + ONC Cures Act EHR interoperability + cross-overlay PHI data-flow coordination. **cell-therapy-supply-chain** (pharma + atmp + transport-hazmat) — Chain of Identity (vein-to-vein for autologous; donor+lot+recipient for allogeneic with 30-yr EU + indefinite US retention) + Chain of Custody per EU GMP Annex 2A §11 + cryogenic UN1977 LN2 dry-shipper IATA PI 202 + Time-Out-of-Storage cumulative budget + 21 CFR 1271 HCT/P Subpart A+C+D+F + autologous failure-mode escalation per FDA CAR-T 2024 Guidance + IATA TTSC + CEIV Pharma carrier qualification. **food-allergen-recall** (food-safety + recall-workflow + privacy) — 9 major allergens per FALCPA + FASTER Act (sesame addition 2023-01-01) + 21 CFR 7 Class I default for undeclared allergens + FSMA §117.135(c)(2) food allergen preventive control + Reportable Food Registry 24-hour reporting + consumer-notification ≥98% Level A effectiveness + privacy-compliant consumer data handling per GDPR Article 6(1)(c) legal-obligation basis + CCPA §1798.145(a)(1) exception. **defense-aerospace-cyber** (aerospace + aerospace-defense + defense-cui + cmmc + cmmc-level-2) — DFARS 7012+7019+7020+7021 stack + MIL-STD-882E joint safety+cyber hazard tracking (Tasks 102+200+300) + ITAR USML Category VIII controlled technical data per 22 CFR §126.18 + airworthiness cybersecurity per DO-326A/DO-356A/DO-355/ED-202A + supply-chain SCRM per NIST SP 800-161 + FY19 NDAA §889 prohibition + counterfeit-parts AS9120/AS6081/GIDEP + DoDI 5000.90 platform cyber resilience + cmmc-level-2 binding. **digital-health-multi-region** (medical-devices + privacy + regulated-ai) — per-jurisdiction regulatory scope determination (FDA SaMD + EU MDR + UK MDR/Future Regulations + Swiss MepV + Canada MDR) + data residency + localisation (GDPR + EU-US DPF + China PIPL + Russia 152-FZ + India DPDP) + multi-region consent (GDPR + HIPAA + state laws + Quebec Law 25 + Ontario PHIPA) + multi-region AI/ML coordination (EU AI Act + FDA AI/ML SaMD + Canada AIDA + UK AI principles) + multi-stream breach notification (HIPAA + GDPR + state AG + MDR §92 + FDA §524B) + ISO 13485 + MDSAP harmonisation + per-jurisdiction vigilance reporting (FDA MDR + EUDAMED + UK Yellow Card + Health Canada Section 59-60 + Swissmedic). All 5 reuse parent-overlay templates. **24-module ultra composite validates** (new depth record beats prior 22-module healthcare ultimate): medical-devices + pharma + combination-product + connected-medical-device + digital-health-multi-region + pharma-sterile + hipaa + privacy + 11 cross-cutting + integrated-management-system + soc-2 + soc-2-type-ii + hitrust-csf + hitrust-r2 + iso-27001-cloud + iso-27001-privacy + iso-37301.
- **Notes:** Shipped at v0.47.0. Registry +3 standards (FD&C §524B + FALCPA + FASTER Act + 21st Century Cures Act — all PUBLIC). Cross-overlay count 3 → 8. Total module count 106 → 111. Each cross-overlay encodes regulatory requirements that emerge specifically at the intersection — combination-of-existing-overlays misses the coordination layer. Particularly natural compositions: `medical-devices + samd + connected-medical-device + privacy + hipaa + regulated-ai + iso-27001 + iso-27001-cloud` (typical SaMD-with-cloud-backend); `pharma + atmp + transport-hazmat + cell-therapy-supply-chain + recall-workflow` (commercial CAR-T manufacturer with vein-to-vein logistics); `food-safety + recall-workflow + privacy + food-allergen-recall + iso-22000` (multi-channel-distribution food brand); `aerospace + aerospace-defense + defense-cui + cmmc + cmmc-level-2 + defense-aerospace-cyber + iso-27001 + iso-27001-cloud` (DoD aerospace prime). Discharges 4 of 5 cross-overlay forward-work items named at 2026-05-25 audit / what's-left analysis (cell-therapy-supply-chain + connected-medical-device + defense-aerospace-cyber + digital-health-multi-region) + adds food-allergen-recall as bonus.

### OQ-112 — HIPAA dedicated cross-cutting overlay (25th cross-cutting; healthcare-specific privacy + security)

- **Tier:** Module
- **Evidence type:** example-tested
- **Status:** :tested
- **Depends_on:** OQ-011, OQ-012, OQ-014, OQ-108
- **Claim:** Open QMS ships a HIPAA dedicated cross-cutting overlay at `modules/hipaa/module.yaml` (version 0.1.0) — 25th cross-cutting overlay. US healthcare-specific privacy + security regime per 45 CFR Parts 160 + 164 + HITECH Act. Distinct from the `privacy` cross-cutting overlay (GDPR + CCPA) because HIPAA's covered-entity (CE) + business-associate (BA) structure + 4-rule architecture (Privacy Rule + Security Rule + Breach Notification Rule + Enforcement Rule) + OCR enforcement regime + Business Associate Agreement (BAA) framework + 4-tier CMP structure + healthcare-specific provisions (TPO permitted uses, de-identification Safe Harbor, NPP, accounting of disclosures, Authorization) don't map cleanly to the GDPR/CCPA framing. 14 clauses across CE/BA scope (§160.103) + uses + disclosures general (§164.502) + permitted without authorisation TPO + 12 enumerated public-interest (§164.508-512) + de-identification + limited datasets (§164.514) + individual rights (§164.520-528) + administrative requirements + Privacy Officer + workforce training (§164.530) + Security Rule administrative safeguards (§164.306 + §164.308) + physical safeguards (§164.310) + technical safeguards (§164.312) + organisational requirements + BAA required content (§164.314 + §164.504(e)) + documentation 6-year retention (§164.316) + Breach Notification Rule with 4-factor risk assessment + 60-day individual notification + media ≥500 + HHS portal (§164.400-414) + Enforcement Rule HITECH 4-tier CMP + criminal penalties (§160 Subpart D + E) + HITECH Promoting Interoperability. Ships 4 HIPAA-specific templates: Notice of Privacy Practices (NPP — §164.520 patient-facing notice with 2024 Reproductive Health Care Privacy Rule additions), Business Associate Agreement (BAA — §164.504(e) + §164.314 full required content + subcontractor flow-down), HIPAA Security Rule Risk Analysis (§164.308(a)(1)(ii)(A) foundational artifact with system inventory + threat catalog + vulnerability assessment + risk treatment + implementation-specification status per §§308/310/312), HIPAA Breach 4-Factor Risk Assessment (§164.402(2) presumption-rebuttal worksheet). **22-module healthcare ultimate composite validates** (new depth record beats prior 20-module): medical-devices + pharma + combination-product + pharma-sterile + hipaa + privacy + iso-27001 + iso-27001-cloud + iso-27001-privacy + regulated-ai + iso-14001 + iso-45001 + iso-50001 + iso-37001 + iso-22301 + iso-31000 + integrated-management-system + soc-2 + soc-2-type-ii + hitrust-csf + hitrust-r2 + iso-37301.
- **Notes:** Shipped at v0.45.0. Registry +2 PUBLIC standards (HIPAA + HITECH). Cross-cutting overlay count: 24 → 25. Total module count 105 → 106. Particularly natural compositions: `hipaa + privacy` (covers PHI under HIPAA AND non-PHI employee/applicant/vendor data under GDPR/CCPA/state laws — most adopters need both); `medical-devices + hipaa + privacy + iso-27001` (connected medical devices processing PHI); `hipaa + hitrust-csf + hitrust-r2` (HITRUST r2 provides validated multi-framework attestation including HIPAA controls); `hipaa + iso-37301 + iso-37301-healthcare + privacy` (compliance-management substrate + healthcare sectoral profile + HIPAA-specific requirements). **Forward work:** 42 CFR Part 2 (substance use disorder) overlay with narrower scope + additional consent; Reproductive Health Care Privacy Rule 2024 attestation template per §164.509; HHS OCR Compliance Investigation response template; Authorization form template per §164.508; Accounting of Disclosures log template per §164.528; Restriction Request log per §164.522(a); ONC certified EHR technology adoption tracker; connected-medical-devices cross-overlay (medical-devices + hipaa + privacy + iso-27001).

### OQ-111 — Cross-overlay batch (3): combination-product + integrated-management-system + food-pharma-grade

- **Tier:** Module
- **Evidence type:** example-tested
- **Status:** :tested
- **Depends_on:** OQ-011, OQ-012, OQ-040, OQ-092
- **Claim:** 3 new cross-overlays at v0.44.0 introducing the cross-overlay module shape — overlays that bind ACROSS specific vertical combinations to encode intersection-specific regulatory requirements (distinct from cross-cutting overlays which apply to any single vertical). **combination-product** (medical-devices + pharma) — FDA 21 CFR Part 4 combination-product CGMP + EU MDR Article 117 device-component NB opinion; 6 clauses covering Primary Mode of Action (PMOA) determination + Request for Designation (RFD) per 21 CFR §3.7 + streamlined single-system CGMP approach per §4.4 (drug-led or device-led) + cross-application registration (NDA/BLA + 510(k)/PMA) + postmarket surveillance coordination (lead-reporter MDR + ADE coordination per §4 Subpart B) + design controls cross-application. **integrated-management-system** (multi-MS coordination) — Annex SL high-level structure leverage + shared policy/objectives + shared context + integrated risk register + combined management review per §9.3 of each MS standard + combined audit programme per ISO 19011:2018 + integrated documented-information control + Annex SL HLS version tracking + coordinated MS-standard-revision transitions. **food-pharma-grade** (chemicals + food-safety + pharma) — food-contact materials regulatory framework + pharma packaging USP standards intersection; 6 clauses across US FCS/FCN regulatory pathways (21 CFR 174-178 + FCN per 21 USC §348(h)) + EU FCM framework (Regulation 1935/2004 Article 3 safety + Article 16 supply-chain Declaration of Compliance) + EU plastic FCM positive list + SMLs (Regulation 10/2011 Annex I + II + III food simulants A-E) + USP packaging chapters (<660>/<661>/<87>/<88>) + extractables/leachables workflow per USP <1663>/<1664> + supply-chain DoC cascade. **20-module ultimate composite validates** (new depth record beats prior 19-module): medical-devices + pharma + combination-product + pharma-sterile + privacy + 8 cross-cutting + integrated-management-system + soc-2 + soc-2-type-ii + hitrust-csf + hitrust-r2 + iso-27001-cloud + iso-27001-privacy.
- **Notes:** Shipped at v0.44.0. Registry +5 standards (21 CFR Part 4 + 21 CFR 174-178 + EU 1935/2004 + EU 10/2011 — all PUBLIC; USP Packaging Chapters commercial). Introduces new module shape: **cross-overlay** — distinct from class overlays (within-vertical rigor delta), sub-overlays (within-cross-cutting tier delta), and cross-cutting overlays (apply to any single vertical). Cross-overlays bind across specific vertical combinations + coordinate cross-QMS requirements. Module count 102 → 105. Cross-cutting overlay count unchanged at 24. Particularly natural compositions: `medical-devices + pharma + pharma-sterile + combination-product` (drug-device combo with sterile parenteral component); `pharma + iso-27001 + iso-14001 + iso-45001 + iso-50001 + integrated-management-system` (full IMS pharma manufacturer); `chemicals + food-safety + pharma + food-pharma-grade` (polymer manufacturer serving both food-pack + pharma-pack downstream).

### OQ-110 — Sub-overlay batch (10): DORA tiers + HITRUST scoping levels + ISO 37301 sectoral profiles

- **Tier:** Module
- **Evidence type:** example-tested
- **Status:** :tested
- **Depends_on:** OQ-011, OQ-104, OQ-109
- **Claim:** 10 new sub-overlays at v0.43.0 extending the sub-overlay shape introduced at v0.42.0 to three additional cross-cutting overlays. **DORA tiers (3):** `dora-ctpp` (Critical Third-Party Provider designation per Chapter V Articles 31-44; Lead Overseer EBA/ESMA/EIOPA direct oversight; JET on-site inspections; corrective recommendations; annual oversight fees); `dora-non-ctpp` (baseline ICT third-party provider scope per Article 30 contractual framework; indirect via financial-entity due diligence; CTPP elevation monitoring); `dora-tlpt` (Threat-Led Penetration Testing per Articles 26-27 + TIBER-EU framework; 3-year minimum cadence; TI Provider + Red Team Provider accreditation; third-party inclusion + cross-border recognition + Article 19 incident-reporting integration). **HITRUST scoping levels (3):** `hitrust-e1` (Essentials 1-year — 44 controls cyber-hygiene baseline; rapid time-to-attestation); `hitrust-i1` (Intermediate 1-year — 182 leading-practices controls; mid-tier assurance); `hitrust-r2` (Risk-based 2-year — 200-2000+ factor-tailored controls + PRISMA maturity scoring + multi-framework crosswalk HIPAA + HITECH + ISO 27001 + NIST + PCI DSS + GDPR; gold standard). **ISO 37301 sectoral profiles (4):** `iso-37301-public-sector` (public-trust mandate + procurement integrity + FOI transparency + UN Convention against Corruption alignment); `iso-37301-financial-services` (prudential + conduct overlay Basel III/IV + Solvency II + MiFID II + AML/CFT/sanctions + consumer protection + DORA/NYDFS coordination); `iso-37301-healthcare` (HIPAA + HITECH + FWA + clinical-quality overlap + research compliance + 340B/pharmacy); `iso-37301-general-business` (Annex SL IMS baseline + employment/labor + product-safety + tax/trade + competition/antitrust). All 10 reuse parent-overlay templates with binding deltas — no new templates. No new registry standards. Cross-cutting overlay count unchanged at 24. Total module count 92 → 102.
- **Notes:** Shipped at v0.43.0. Extends sub-overlay-shape decomposition pattern to 3 additional cross-cutting overlays. Total sub-overlays after v0.43.0: 26 (16 from v0.42.0 + 10 new). Particularly natural compositions: `dora + dora-ctpp + dora-tlpt` (CTPP-designated provider with TLPT scope obligations); `hitrust-csf + hitrust-r2 + privacy` (healthcare SaaS gold-standard certification); `iso-37301 + iso-37301-financial-services + dora + dora-non-ctpp` (financial entity full-compliance posture); `iso-37301 + iso-37301-healthcare + privacy + hitrust-csf + hitrust-r2` (healthcare-provider comprehensive compliance + HIPAA-aligned attestation). ISO 37301 sectoral profiles are softer-defined than DORA + HITRUST tiers — 37301 itself doesn't formalise sectoral tiers; profiles encode community practice + sectoral compliance-obligation catalogues.
...
- v0.43.0: **Sub-overlay batch (10): DORA tiers + HITRUST scoping levels + ISO 37301 sectoral profiles.** OQ-110 NEW `:tested` — extends sub-overlay-shape pattern to 3 additional cross-cutting overlays. 3 DORA tiers (dora-ctpp + dora-non-ctpp + dora-tlpt per TIBER-EU) + 3 HITRUST scoping levels (hitrust-e1 Essentials 44 controls + hitrust-i1 Intermediate 182 controls + hitrust-r2 Risk-based 2-year 200-2000+ controls + PRISMA + multi-framework crosswalk) + 4 ISO 37301 sectoral profiles (public-sector + financial-services + healthcare + general-business). All 10 reuse parent-overlay templates. No new registry standards. Cross-cutting overlay count unchanged at 24. Total module count 92 → 102. Total sub-overlay count 16 → 26. CI extended (+15 validate steps). Engine 0.42.0 → 0.43.0. Status counts: 6 `:verified` / 83 `:tested` / 5 `:argued` / 0 `:open` (total 94).
- v0.44.0: **Cross-overlay batch (3): combination-product + integrated-management-system + food-pharma-grade.** OQ-111 NEW `:tested` — introduces new module shape (cross-overlay) binding ACROSS specific vertical combinations + encoding intersection-specific requirements. combination-product (medical-devices + pharma — FDA 21 CFR Part 4 PMOA + streamlined CGMP + EU MDR Article 117 NB opinion + cross-application registration + postmarket coordination). integrated-management-system (multi-MS — Annex SL HLS leverage + combined management review + combined audit programme per ISO 19011 + integrated risk register). food-pharma-grade (chemicals + food-safety + pharma — US FCS/FCN + EU FCM framework + plastic FCM positive list + USP packaging chapters + E&L workflow + supply-chain DoC). All 3 reuse parent-overlay templates. Registry +5 standards (21 CFR Part 4 + 21 CFR 174-178 + EU 1935/2004 + EU 10/2011 + USP Packaging Chapters; 4 PUBLIC, 1 commercial). Total module count 102 → 105. **20-module ultimate composite validates** (new depth record). Engine 0.43.0 → 0.44.0. Status counts: 6 `:verified` / 84 `:tested` / 5 `:argued` / 0 `:open` (total 95).
- v0.45.0: **HIPAA dedicated cross-cutting overlay.** OQ-112 NEW `:tested` (hipaa — US healthcare-specific privacy + security per 45 CFR Parts 160 + 164 + HITECH Act). 14 clauses across CE/BA scope + uses + disclosures + de-identification + individual rights + administrative requirements + Security Rule administrative + physical + technical safeguards + BAA + breach notification 4-factor + Enforcement Rule HITECH 4-tier CMP. 4 new HIPAA-specific templates: NPP (with 2024 Reproductive Health Care Privacy Rule additions) + BAA (full §164.504(e) + §164.314 content) + Security Rule Risk Analysis (§164.308(a)(1)(ii)(A)) + Breach 4-Factor Risk Assessment (§164.402(2)). Registry +2 PUBLIC standards (HIPAA + HITECH). Cross-cutting overlay count 24 → 25. Total module count 105 → 106. **22-module healthcare ultimate composite validates** (new depth record). Engine 0.44.0 → 0.45.0. Status counts: 6 `:verified` / 85 `:tested` / 5 `:argued` / 0 `:open` (total 96).
- v0.46.0: **HIPAA satellite templates — discharges OQ-112 forward-work bucket.** No spec entries added; **1 new clause** added to OQ-112 hipaa module (HIPAA-164-509-reproductive-health-attestation per HHS Final Rule 89 FR 32976) + **5 new HIPAA satellite templates**: HIPAA-AUTHORIZATION (§164.508), ACCOUNTING-OF-DISCLOSURES-LOG (§164.528), RESTRICTION-REQUEST-LOG (§164.522(a) permissive + HITECH §13405(a) MANDATORY + §164.522(b) confidential communications), OCR-INVESTIGATION-RESPONSE (Resolution Agreement + CAP + CMP §160.404 + 42 USC §1320d-6 + Wall of Shame), REPRODUCTIVE-HEALTH-ATTESTATION (§164.509). hipaa module 14→15 clauses + 5→10 templates. Total templates 96→101. No new registry standards. **Engine version corrected 0.43.0 → 0.46.0** — the v0.44 + v0.45 engine bumps had silently failed in earlier release commits despite all other release content shipping; corrected here. Status counts unchanged: 6 `:verified` / 85 `:tested` / 5 `:argued` / 0 `:open` (total 96).
- v0.47.0: **Cross-overlay batch (5): connected-medical-device + cell-therapy-supply-chain + food-allergen-recall + defense-aerospace-cyber + digital-health-multi-region.** OQ-113 NEW `:tested` — extends cross-overlay shape (introduced at v0.44.0 OQ-111) to 5 additional vertical-intersection regulatory layers. connected-medical-device (med-dev + regulated-ai + privacy + iso-27001 + hipaa — FDA §524B cyber-device + EU MDR Annex I §17.2 + AI/ML PCCP + SBOM + ONC Cures Act + cross-overlay PHI). cell-therapy-supply-chain (pharma + atmp + transport-hazmat — COI + COC vein-to-vein + cryogenic UN1977 + TOS + 21 CFR 1271 + autologous failure-mode). food-allergen-recall (food-safety + recall-workflow + privacy — 9 major allergens FALCPA + FASTER + Class I + RFR + ≥98% effectiveness + privacy-compliant consumer notification). defense-aerospace-cyber (aerospace + aerospace-defense + defense-cui + cmmc + cmmc-level-2 — DFARS 7012/7019/7020/7021 stack + MIL-STD-882E joint safety+cyber + ITAR + airworthiness cyber DO-326A + SCRM + DoDI 5000.90). digital-health-multi-region (med-dev + privacy + regulated-ai — per-jurisdiction scope + data residency + multi-region consent + multi-region AI/ML + multi-stream breach + ISO 13485+MDSAP harmonisation + per-jurisdiction vigilance). Registry +3 PUBLIC standards (FD&C §524B + FALCPA+FASTER Act + Cures Act). Cross-overlay count 3 → 8. Total module count 106 → 111. **24-module ultra composite validates** (new depth record beats prior 22-module healthcare ultimate). Engine 0.46.0 → 0.47.0. Status counts: 6 `:verified` / 86 `:tested` / 5 `:argued` / 0 `:open` (total 97).
- v0.48.0: **US State Privacy Umbrella cross-cutting overlay.** OQ-114 NEW `:tested` (us-state-privacy — 26th cross-cutting overlay covering ~14 US state comprehensive privacy laws using VCDPA template family). 10 clauses across applicability thresholds + controller/processor framework + 6 common consumer rights with right-to-appeal + sensitive data divergence + UOOM (GPC honoring) per-state + Data Protection Assessment + cure period + AG enforcement + controller obligations + Maryland MODPA stricter + Florida FDBR narrow scope. Covers Virginia VCDPA + Colorado CPA + Connecticut CTDPA + Utah UCPA + Texas TDPSA + Oregon OCPA + Montana MCDPA + Iowa ICDPA + Delaware DPDPA + NH NHPA + NJ NJDPA + Tennessee TIPA + Minnesota MCDPA + Maryland MODPA + Indiana INCDPA + Florida FDBR. 2 new templates: US-STATE-PRIVACY-MATRIX (state-by-state divergence tracker) + DATA-PROTECTION-ASSESSMENT (state-level DPA parallel to GDPR DPIA). Plus 2 extensions to PRIVACY-POLICY + DPA templates. Registry +1 PUBLIC standard (US State Privacy Laws). Cross-cutting overlay count: 25 → 26. Total module count 111 → 112. Engine 0.47.0 → 0.48.0. Status counts: 6 `:verified` / 87 `:tested` / 5 `:argued` / 0 `:open` (total 98).
- v0.57.0: **Non-technical UX batch (P3).** OQ-121 NEW `:tested` (Gap-tier). 5 new GitHub Issue Forms (`training-completion.yml` + `document-review.yml` + `access-review.yml` + `restoration-test.yml` + `regulatory-review.yml`) align 1:1 with SOPs shipped at OQ-120 — completing the SOP-template → operating-record-form lifecycle. 5 new role/process guides (`onboarding.md` + `role-quality-manager.md` + `role-engineer.md` + `document-routing.md` + `capa-lifecycle.md`) bring docs/guide/ from 12 → 17. CAPA-lifecycle guide formalises 7-state state machine. Engine 0.56.0 → 0.57.0. Status counts: 6 `:verified` / 94 `:tested` / 5 `:argued` / 0 `:open` (total 105). Forward-work after v0.57.0: 8 closed · 6 open (P2 + P15 hard · P4 + P5 + P9 + P10 + P11 medium).
- v0.56.0: **P15 added to forward work (integration-architecture).** No spec entries; doc-only refinement of OQ-119. Independent reviewer flagged 2026-05-25: *"Risk management should not be a separate artifact. The future is integrated: requirements ↔ hazards ↔ mitigations ↔ testing ↔ CAPA ↔ complaints ↔ post-market surveillance. That connectedness is where modern systems win."* Added P15 (hard effort) to `docs/compliance-architecture.md` forward-work as a new "Integration-architecture priorities" subsection — instance-level cross-record trace network parallel to (but distinct from) the clause-level trace already enforced by OQ-001 + OQ-067. Effort distribution updated: 2 hard (P2 + P15) · 6 medium · 0 light (8 open total post-v0.55.0). Engine 0.55.0 → 0.56.0. Status counts unchanged: 6 `:verified` / 93 `:tested` / 5 `:argued` / 0 `:open` (total 104).
- v0.55.0: **Light-batch hardening release.** OQ-120 NEW `:tested` (Gap-tier). Single batched release closing the 6 light-effort forward-work priorities identified at v0.54.0. **P6 lockfile** (`engine/uv.lock` + `uv sync --frozen` CI gate); **P7 signed release tags** (`docs/guide/release.md` mandates `git tag -s`; verification embedded in P8 workflow); **P8 CI log archival** (`.github/workflows/release-artifact.yml` captures trace + coverage + signatures + pytest + lint + module-count + tag-signature per tag push); **P12 identity-mapping SOP template** (`templates/qms-policy/IDENTITY-MAPPING-SOP-TEMPLATE.md`); **P13 backup/restore SOP template + 13-step annual restoration-test runbook** (`templates/qms-bcms/BACKUP-RESTORE-SOP-TEMPLATE.md`); **P14 regulatory-review cadence** (`BUSINESS/regulatory_review_cadence.md` with honest 0/116 review-debt baseline + 8-module first-tier review-target list). Engine 0.54.0 → 0.55.0. Status counts: 6 `:verified` / 93 `:tested` / 5 `:argued` / 0 `:open` (total 104). Forward-work after v0.55.0: 6 closed · 7 open (P2 hard · P3 + P4 + P5 + P9 + P10 + P11 medium).
- v0.54.0: **Forward-work effort qualifiers.** No spec entries; per-priority `light/medium/hard` effort calibration added to all three tables in `docs/compliance-architecture.md` forward-work section. Brief calibration legend at top (light ≤ ½ day; medium ½ to 2 days; hard 2+ days multi-session). Current distribution: 1 hard (P2 validation package) · 6 medium (P3 + P4 + P5 + P9 + P10 + P11) · 6 light (P6 + P7 + P8 + P12 + P13 + P14). Informs sequencing decisions; not a commitment. Engine 0.53.0 → 0.54.0. Status counts unchanged: 6 `:verified` / 92 `:tested` / 5 `:argued` / 0 `:open` (total 103).
- v0.53.0: **Reviewer-feedback integration — forward-work section expanded.** No new spec entries; documentation refinement of the OQ-119 deliverable. Integrated independent reviewer's technical assessment into `docs/compliance-architecture.md` forward-work section. Added 10 new priorities P5–P14 across two clusters: **engine-hardening** (P5 replace shell/grep doc-control workflow with schema-validated YAML parser + hard-fail version drift + signed audit artifact per PR; P6 lockfile with hashed deps via `uv`; P7 signed release tags; P8 CI log archival per release; P9 template schema validation; P10 negative-path test suite for bad modules) and **adopter-deployment governance** (P11 `verify-deployment` script for branch protection + CODEOWNERS + signed-commits verification; P12 HR-to-GitHub identity-mapping SOP template; P13 backup/restore SOP template + restoration-test discipline; P14 third-party regulatory review cadence for module crosswalks). Plus "what reviewer flagged that is already addressed" transparency table (5 items). Engine 0.52.0 → 0.53.0. Status counts unchanged: 6 `:verified` / 92 `:tested` / 5 `:argued` / 0 `:open` (total 103).
- v0.52.0: **Compliance architecture trust-gate document (adopter-feedback P1).** OQ-119 NEW `:tested` (Gap-tier). Shipped `docs/compliance-architecture.md` — 11-section comprehensive trust-gate document mapping 10 highest-frequency adopter-evaluator topics (21 CFR Part 11 + EU GMP Annex 11 + ISO 13485 + audit trails + e-signatures + permissions + validation + backup/recovery + record retention + cybersecurity + system administration controls) to architecture components + spec entries + evidence files + adopter responsibilities. Includes "How to use this document at audit" workflow + honest disclaimers + forward-work section acknowledging remaining adopter-feedback priorities (P2 validation package templates + P3 non-technical UX + P4 opinionated startup-stage presets). Engine 0.51.0 → 0.52.0. Status counts: 6 `:verified` / 92 `:tested` / 5 `:argued` / 0 `:open` (total 103).
- v0.51.0: **Polishing pass.** No spec entries; bookkeeping release. A0-A6 cross-audit run post-v0.50.0; zero findings (see `audit_2026-05-25_v0_50.md`). Omnibus companion shipped for the v0.48-v0.51 arc (`companion_v0_48_to_51.md`). `companion_index.md` updated. README + catalog version-refs bumped to v0.51.0. Engine 0.50.0 → 0.51.0. Status counts unchanged: 6 `:verified` / 91 `:tested` / 5 `:argued` / 0 `:open` (total 102). Closes the v0.46-v0.50 build arc cleanly.
- v0.50.0: **Final cross-overlay batch (4).** OQ-118 NEW `:tested` — completes the cross-overlay shape across the remaining major vertical intersections. **automotive-supply-chain** (automotive + automotive-asil-d + automotive-cal-4 + recall-workflow — DIA + CIA + AIAG PPAP 18-element + UN R155 + R156 + NHTSA Part 573 + tier-N flow-down + software-supplier special considerations); **clinical-trial-multi-region** (pharma + privacy + hipaa + digital-health-multi-region — ICH E6(R3) GCP + EU CTR 536/2014 + CTIS + 21 CFR 312 IND + DCT + multi-region PHI coordination + parallel IRB/IEC + RBM + pharmacovigilance multi-region); **banking-resilience** (iso-37301-financial-services + dora + nist-csf + iso-22301 — DORA + FFIEC cross-regime + NIST CSF backbone + multi-stream incident reporting + TLPT alignment + CTPP + ISO 22301 BIA + ICT BCP); **utility-cybersecurity** (manufacturing + iso-27001 + nist-csf — NERC CIP-002 through CIP-014 + TSA Pipeline SD + CISA 16 sectors + EU NIS2 + IEC 62443 + CIRCIA + Energy Subsector). All 4 reuse parent-overlay templates. Registry +5 PUBLIC standards (ICH E6(R3) + EU CTR 536/2014 + NERC CIP + EU NIS2 + FFIEC IT Handbook). Cross-overlay count 8 → 12. Total module count 112 → 116. CI extended (+8 validate steps). Engine 0.49.0 → 0.50.0. Status counts: 6 `:verified` / 91 `:tested` / 5 `:argued` / 0 `:open` (total 102).
- v0.49.0: **Engine adopter-features — 3 new CLI subcommands.** OQ-115 + OQ-116 + OQ-117 all NEW `:tested`. (1) **`openqms coverage`** (OQ-115) — per-module + aggregate coverage metrics; --threshold N CI regression gate; aggregate coverage = 100.0% across 112 modules / 838 clauses / 0 orphans at audit. (2) **`openqms crosswalk`** (OQ-116) — clause-crosswalk between modules; identifies same-(standard, section) overlap; per-module shared-vs-own-only counts. (3) **`openqms jurisdictions-query`** (OQ-117) — registry standards by jurisdiction with publisher-inference (hardcoded _PUBLISHER_TO_JURISDICTIONS table covering 30+ major publishers); FDA query returns 18 standards at audit. 13 new pytest tests in test_coverage.py + test_crosswalk.py + test_jurisdictions_query.py (test count 116 → 129). CI extended with coverage-100% regression gate + crosswalk smoke + jurisdictions-query FDA/EU smoke. No new modules; no new registry standards. First Architecture-tier additions since OQ-067 (trace, v0.39.0). Engine 0.48.0 → 0.49.0. Status counts: 6 `:verified` / 90 `:tested` / 5 `:argued` / 0 `:open` (total 101).
