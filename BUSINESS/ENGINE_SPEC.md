...
- **Notes:** Decision documented at v0.2.1 in `docs/guide/complaints.md`. The intake template (`.github/ISSUE_TEMPLATE/complaint.yml`, OQ-069) captures only PHI-redacted triage metadata and includes a required PHI-handling confirmation. Status stays `:argued` — this is an architecture decision, not a mechanically-verifiable property; upgrading to `:tested` would require a runtime check that no PHI patterns appear in issue bodies, which is out of scope.
...
### OQ-067 — Repository-wide trace matrix is forward work, not Phase 0

- **Tier:** Gap
- **Evidence type:** manual
- **Status:** :argued
- **Depends_on:** OQ-031, OQ-010
- **Claim:** A repository-wide traceability matrix across the full issue/PR linkage graph is forward work and will ship with the Open QMS generator engine. The per-PR snippet produced by `traceability.yml` is the Phase-0 traceability surface; no standalone matrix-generation script exists or is planned outside the engine.
- **Notes:** At v0.1.0 this entry tracked a broken reference in `docs/guide/traceability.md` to a non-existent `./scripts/generate-trace-matrix.sh`. Reference removed at v0.1.1 (commit pending); doc now points at the engine-driven forward path. Open observation: the YAML job ID `generate-trace-matrix` in `.github/workflows/traceability.yml:43` echoes the same misleading name; the job's display name ("Generate traceability snippet") is accurate but the YAML ID is not. Not changed here — renaming touches GitHub Actions run history and any branch-protection required-checks configured against the old name. Defer to a deliberate cleanup pass.

### OQ-068 — Template subdirectory population (v0.10.0)

- **Tier:** Gap
- **Evidence type:** example-tested
- **Status:** :tested
- **Depends_on:** OQ-038
- **Claim:** Of the original 15 `templates/` subdirectories with no content (v0.1.0 spec text said 11 — historic miscount, corrected at v0.3.0), **13 are populated** as of v0.10.0: `product-dhf/{risk-management,verification,validation,technical-file,usability,packaging,gspr,clinical}/`, `product-sw/{requirements,architecture,soup-register,test,release}/`, plus `qms-suppliers/` and `qms-management-review/`. **2-4 dirs remain placeholder-only**, intentionally adopter-defined: `qms-{capa,forms,training}/` (org-level workflows where adopters define their own forms/records) and `product-dhf/design-outputs/` (product-specific structure that varies by device family).
- **Notes:** 21 document templates ship today (was 3 at v0.1.0; 11 at v0.3.0; 14 at v0.8.0; 18 at v0.9.0). Each populated template includes the standard document-control frontmatter and is bound to clauses in the medical-devices module manifest. The clinical/ and gspr/ subdirs added at v0.10.0 are new (not among the original 15 empty subdirs); they were created as part of the EU MDR Annex I + Annex XIV closure.
